...
- **Creating User Account:** The process of adding a new user to the system with specified attributes such as username, password, and home directory.
	  ...
	  def create_user(username, password):
	      try:
	        subprocess.run(['powershell', '-Command', f'New-LocalUser -Name {username} -Password (ConvertTo-SecureString "{password}" -AsPlainText -Force)'])
	...
	The script uses the `subprocess` module to execute a PowerShell command, creating a new local user account on a Windows system. The `create_user` function constructs a PowerShell command with the provided username and password, utilizing the `New-LocalUser` cmdlet. Upon successful creation, a confirmation message is displayed; otherwise, an error message is shown.
...
- **Modifying User Account:** Making changes to an existing user account, such as updating user information or modifying permissions.
	  ...
	  def modify_user(username, new_password):
	    try:
	        subprocess.run(['powershell', '-Command', f'Set-LocalUser -Name {username} -Password (ConvertTo-SecureString "{new_password}" -AsPlainText -Force)'])
	...
	The script employs the `subprocess` module to execute a PowerShell command, altering the password of an existing local user account on a Windows system. The `modify_user` function accepts parameters for the username and the new password, constructing a PowerShell command using string formatting. If the operation succeeds, a confirmation message is shown.
...
- **Deleting User Account:** Removing an existing user account from the system, including all associated files and directories.
		...
		def delete_user(username):
		    try:
		        subprocess.run(['powershell', '-Command', f'Remove-LocalUser -Name {username} -Force'])
...
The script uses the `icacls` command to modify file permissions in Windows. The `/grant` option specifies user and permissions, while the `permission` parameter allows customization using flags like F, M, RX, R, W, D.

<div class="div-blue"> <span class="alert-header">Note:</span> <span class="alert-body"> The commands used in the examples above works on the Windows OS.</span> </div>
...
You can use the `sys` module in Python to configure software settings programmatically on Windows, particularly for command-line arguments handling.
